...
use tokio_rustls::{
    TlsAcceptor, rustls::{ ServerConfig, ClientConfig}
};
        ...
        async fn handle_raw_http_request(
            ...
            let resp = match resp {
                ...
                Err(e) => {
                    ...
                    return Ok(Response::builder()
                        .status(StatusCode::BAD_GATEWAY)
                        .body(Full::new(Bytes::from("Failed to get response from target server")))
                        .unwrap());
                }
            };
            ...
            let response = Response::builder()
                ...
                .body(Full::new(Bytes::from(resp_body))) // 响应体内容
                .unwrap();
            Ok(response)
        }
